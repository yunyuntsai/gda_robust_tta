...
def load_imagenets_data(
    *,
    ...
    for i in range(len(classes)): 
        print('idx : ', i , 'j: ', j, 'classes: ', classes[i])
        if  classes[i] == j  and cnt < 1 and j < 1000:
            subset_idx.append(i)
            cnt += 1
        elif cnt == 1 and classes[i] == j:
           j += 1
           cnt = 0
        else:
            continue
...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.local_images = np.array(image_paths)[subset_idx][::num_shards]
        # self.local_images = image_paths[shard:][::num_shards]
        self.local_classes = None if classes is None else classes[subset_idx][::num_shards]
