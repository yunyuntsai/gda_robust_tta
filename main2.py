...
from utils import load_imagenetC, load_imagenetR, RotAug, RotationTransform
import os
import math
import csv
...
def cond_fn(x, t, y=None):
    with torch.enable_grad():
        ...
        if args.guided_type=='weighted-marginal':
            ...
            m_loss, logits = weighted_marginal_entropy(output, orig_output)
            ...
            tr_inputs = torch.squeeze(torch.stack(tr_inputs),1).to(device)     
            output, _ = resnet_model(tr_inputs)
            m_loss, logits = marginal_entropy(output)
            loss =  m_loss * 100 + g_loss * 5000 + feat_loss * 100 + mse_loss * 100 + z_loss * 1000
            ...
            x_loss = xent_loss(output, target)
...
def get_args():
    ...
    parser.add_argument('--corruption', default='gaussian_noise', type=str)
...
corr_dataset = load_imagenetC(args.corruption, args.severity) 
corr_dataset  = corr_dataset.imgs
...
# eval(corr_dataset, resnet_model)
subset_idx = []
for i in range(len(corr_dataset)):
    if i % 50 < 1: #1-K classes, 5 images/class
        subset_idx.append(i)
...
for idx in subset_idx:
    init_img_path, target =  corr_dataset[idx]
        ...
        for j, sample in tqdm.tqdm(enumerate(samples)):
            ...
            if j % 40 == 0 or cur_t == -1:
                ...
                for k, image in enumerate(sample['pred_xstart']):
                    filename = f'samples/{args.corruption}/img_{idx}_progress_{i * batch_size + j:05}.png'
                    # init_image = init_image.resize((model_config['image_size'], model_config['image_size']), Image.LANCZOS)
                    ...
                    if j == 0:
                        ...
                        if args.ensemble:
                            ...
                            print('final adapt ent: ', ent1.item(), 'orig ent2: ', ent2.item())

                            adapt_ent.append(ent1.cpu().detach().item())
                            orig_ent.append(ent2.cpu().detach().item())
                            if ent2 < ent1:
    ...
    test_n += 1
    print('test_n: {} before adapt: {} after adapt: {}'.format(test_n, orig_acc/test_n, adapt_acc/test_n))

    
    # if test_n % 100 == 0:
    #     with open(os.path.join(f'./output/s3_noguided/{args.corruption}_{args.guided_type}_{args.skip_timesteps}_result.csv'), 'a') as f: 
    #         writer = csv.writer(f)
    #         writer.writerow([args.guided_type, args.ensemble, args.skip_timesteps, p_source, test_n, orig_acc/test_n, adapt_acc/test_n])

    if test_n == 200:
        with open(f'./loss_histogram/{args.corruption}_{args.guided_type}_result.npy', 'wb') as f:
            np.save(f, np.array([orig_ent, adapt_ent]))